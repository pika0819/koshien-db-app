                ...
                if len(selection.selection.rows) > 0:
                    ...
                    with tab2:
                        m_query = f"""
                            ...
                            ORDER BY 
                                CASE WHEN Grade = '監督' THEN 1 ELSE 0 END,
                    ...
                    with tab3:
                        ...
                        try:
                            ...
                            col_conf = {"年度": st.column_config.NumberColumn(format="%d")}
                            
                            st.dataframe(display_h, use_container_width=True, hide_index=True, column_config=col_conf)
                        except:
                            st.info("過去の出場履歴なし")
        ...
        if not df_s.empty:
            ...
            if sel:
                ...
                try:
                    ...
                    # カラム変更と設定を分離して記述
                    df_disp = df_h.rename(columns={'Year':'年度','Season':'季','Rank':'成績','History_Label':'情報'})
                    conf = {"年度": st.column_config.NumberColumn(format="%d")}
